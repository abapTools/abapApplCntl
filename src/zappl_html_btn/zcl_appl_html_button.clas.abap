...
CLASS zcl_appl_html_button DEFINITION
  ...
  PUBLIC SECTION.
    ...
    INTERFACES zif_appl_html_button .
    ...
    ALIASES get_btn_fields
      FOR zif_appl_html_button~get_btn_fields .
    ALIASES get_btn_type
      FOR zif_appl_html_button~get_btn_type .
    ALIASES set_active
      FOR zif_appl_html_button~set_active .
...
*== set user command
  ...
  METHOD read_btn_db.
    SELECT SINGLE * FROM zappl_btn_types
        INTO CORRESPONDING FIELDS OF s_btn_fields
        WHERE type EQ i_btn_type.
  ENDMETHOD.
